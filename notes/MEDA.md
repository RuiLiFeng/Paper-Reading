...
proof:$x^TAy=x^TU^T\Sigma Vy=\hat{x}^T\Sigma\hat{y}=\sum_{i=1}^m\lambda_i\hat{x}_i\hat{y}_i\leq|\lambda_{max}|$ . The equality is taken when $j=\arg\max_{i=1,...,m}\vert\lambda_i\vert,\vert\hat{x}_j\vert,\vert\hat{y}_j\vert=1$, that is $Ux=(0,...,1,0,..,0)^T,Vy=(0,...,1,0,...,0)^T$, which means x and y are the j-th row of U,V correspondingly.
